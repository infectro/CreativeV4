	...
	if user_id and args[1] then
		if vRP.hasPermission(user_id,"servicePolice") then
			...
			if nuser_id > 0 then
				...
				TriggerClientEvent("Notify",source,"sucesso","Limpeza efetuada.",5000)
			end
		end
	end
end)
...
function cRP.initPrison(nuser_id,services,fines,text)
	...
	if user_id then
		if actived[user_id] == nil then
			...
			TriggerClientEvent("Notify",source,"sucesso","Prisão efetuada.",5000)
...
function cRP.initFine(nuser_id,fines,text)
	...
	if user_id and fines > 0 then
		if actived[user_id] == nil then
			...
			TriggerEvent("discordLogs","Police","**Por:** "..parseFormat(user_id).."\n**Passaporte:** "..parseFormat(nuser_id).."\n**Multa:** $"..parseFormat(fines).."\n**Horário:** "..os.date("%H:%M:%S").."\n**Motivo:** "..text,2316674)
			TriggerClientEvent("Notify",source,"sucesso","Multa aplicada.",5000)
...
function cRP.resetPrison()
	...
	if user_id then
		...
		for k,v in pairs(policeResult) do
			async(function()
				TriggerClientEvent("Notify",v,"importante","Recebemos a informação de um fugitivo da penitenciária.",5000)
			end)
		end
...
function cRP.reducePrison()
	...
	if user_id then
		...
		if parseInt(consult[1]["prison"]) <= 0 then
			vCLIENT.syncPrison(source,false,true)
			vRP.execute("characters/fixPrison",{ user_id = parseInt(user_id) })
			TriggerClientEvent("Notify",source,"sucesso","Serviço acabou, esperamos não vê-lo novamente.",5000)
		else
			vCLIENT.asyncServices(source)
			TriggerClientEvent("Notify",source,"importante","Restam <b>"..parseInt(consult[1]["prison"]).." serviços</b>.",5000)
		end
	end
end
-----------------------------------------------------------------------------------------------------------------------------------------
...
function cRP.checkKey()
	...
	if user_id then
		...
		if parseInt(#policeResult) <= 5 then
			TriggerClientEvent("Notify",source,"importante","Sistema indisponível no momento.",5000)
			return false
		end
