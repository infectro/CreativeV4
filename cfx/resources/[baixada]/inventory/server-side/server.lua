	...
	if user_id then
		if activeItens[user_id] == nil and not vPLAYER.getHandcuff(source) and not vRP.wantedReturn(user_id) then
			if noFull[nameItem] then
				TriggerClientEvent("inventory:Update",source,"updateMochila")
				TriggerClientEvent("Notify",source,"importante","Armazenamento proibido.",5000)
				return
			end
...
function cRP.stealTrunk(entity)
	...
	local user_id = vRP.getUserId(source)
	if user_id then
			if entidade[entity[3]] ~= true then
					...
					if parseInt(random) >= 85 then
							TriggerClientEvent("Notify",source,"amarelo","Nada encontrado.",3000)
							vRPclient._stopAnim(source,false)
					elseif parseInt(random) >= 0 and parseInt(random) <= 84 then
			...
			if taskResult then
				local random = math.random(100)
				if parseInt(random) >= 30 then
					vRP.giveInventoryItem(user_id,"goldring",math.random(8),true)
				elseif parseInt(random) >= 61 and parseInt(random) <= 80 then
					vRP.giveInventoryItem(user_id,"spray01",math.random(8),true)
				elseif parseInt(random) >= 41 and parseInt(random) <= 60 then
					vRP.giveInventoryItem(user_id,"spray02",math.random(6),true)
				elseif parseInt(random) >= 26 and parseInt(random) <= 40 then
					vRP.giveInventoryItem(user_id,"spray03",math.random(5),true)
				elseif parseInt(random) >= 10 and parseInt(random) <= 25 then
					vRP.giveInventoryItem(user_id,"brush",math.random(5),true)
				elseif parseInt(random) <= 5 and parseInt(random) <= 25 then
					vRP.giveInventoryItem(user_id,"rimel",math.random(3),true)
				elseif parseInt(random) >= 56 and parseInt(random) <= 65 then
					vRP.giveInventoryItem(user_id,"soap",math.random(3),true)
				elseif parseInt(random) >= 46 and parseInt(random) <= 55 then
					vRP.giveInventoryItem(user_id,"sneakers",math.random(3),true)
				elseif parseInt(random) >= 36 and parseInt(random) <= 45 then
					vRP.giveInventoryItem(user_id,"dildo",math.random(3),true)
				elseif parseInt(random) >= 26 and parseInt(random) <= 35 then
					vRP.giveInventoryItem(user_id,"eraser",math.random(3),true)
				elseif parseInt(random) >= 1 and parseInt(random) <= 25 then
					vRP.giveInventoryItem(user_id,"deck",math.random(4),true)
				elseif parseInt(random) >= 61 and parseInt(random) <= 80 then
					vRP.giveInventoryItem(user_id,"dices",math.random(8),true)
				elseif parseInt(random) >= 41 and parseInt(random) <= 60 then
					vRP.giveInventoryItem(user_id,"floppy",math.random(6),true)
				elseif parseInt(random) >= 26 and parseInt(random) <= 40 then
					vRP.giveInventoryItem(user_id,"domino",math.random(5),true)
				elseif parseInt(random) >= 10 and parseInt(random) <= 25 then
					vRP.giveInventoryItem(user_id,"horseshoe",math.random(5),true)
				elseif parseInt(random) <= 5 and parseInt(random) <= 25 then
					vRP.giveInventoryItem(user_id,"legos",math.random(3),true)
				elseif parseInt(random) >= 56 and parseInt(random) <= 65 then
					vRP.giveInventoryItem(user_id,"ominitrix",math.random(3),true)
				elseif parseInt(random) >= 46 and parseInt(random) <= 55 then
					vRP.giveInventoryItem(user_id,"teddy",math.random(3),true)
				elseif parseInt(random) >= 36 and parseInt(random) <= 45 then
					vRP.giveInventoryItem(user_id,"pager",math.random(3),true)
				elseif parseInt(random) >= 26 and parseInt(random) <= 25 then
					vRP.giveInventoryItem(user_id,"card05",math.random(3),true)
				elseif parseInt(random) >= 1 and parseInt(random) <= 25 then
					vRP.giveInventoryItem(user_id,"card04",math.random(3),true)
				elseif parseInt(random) >= 1 and parseInt(random) <= 25 then
					vRP.giveInventoryItem(user_id,"card03",math.random(3),true)
				elseif parseInt(random) >= 1 and parseInt(random) <= 25 then
					vRP.giveInventoryItem(user_id,"card02",math.random(4),true)
				elseif parseInt(random) >= 1 and parseInt(random) <= 25 then
					vRP.giveInventoryItem(user_id,"card01",math.random(4),true)
							vRPclient._stopAnim(source,false)
				end
			end
		end
		...
		if math.random(100) >= 65 then
			...
			TriggerClientEvent("Notify",source,"azul","As autoridades foram notificadas da tentativa de roubo.",5000)
			local x,y,z = vRPclient.getPositions(source)
			local copAmount = vRP.numPermission("Police")
			for k,v in pairs(copAmount) do
				async(function()
					TriggerClientEvent("NotifyPush",v,{ time = os.date("%H:%M:%S"), code = 'QRU', title = "Roubo a Porta Malas", x = x, y = y, z = z, criminal = "Alarme do Veiculo", rgba = {160,108,15} })
				end)
			end
		end
		
		entidade[entity[3]] = false
			else
					TriggerClientEvent("Notify",source,"amarelo","Esse veiculo já foi roubado.",3000)
			end
	end
end
-----------------------------------------------------------------------------------------------------------------------------------------
				...
				if dropList[gridZone][dropId] ~= nil then
					if (vRP.inventoryWeight(user_id) + (itemWeight(dropList[gridZone][dropId]["key"]) * parseInt(pickupAmount))) <= vRP.getBackpack(user_id) then
						...
						if vRP.checkMaxItens(user_id,dropList[gridZone][dropId]["key"],pickupAmount) then
							TriggerClientEvent("Notify",source,"importante","Limite atingido.",3000)
							TriggerClientEvent("inventory:Update",source,"updateMochila")
							return
						end
			...
			if itemType(totalName) == "Usável" or itemType(totalName) == "Animal" then
				...
				if nameItem == "newchars" then
					if vRP.tryGetInventoryItem(user_id,totalName,1,false,slot) then
						...
						TriggerClientEvent("Notify",source,"sucesso","Personagem liberado.",5000)
					end
				return end
					...
					if string.len(initCheck) == 3 and string.len(finiCheck) == 3 then
						if not vRP.userPhone(firstNumber.."-"..lastNumber) then
							if vRP.tryGetInventoryItem(user_id,totalName,1,true,slot) then
								...
								TriggerClientEvent("Notify",source,"sucesso","Telefone atualizado.",5000)
							end
						else
							TriggerClientEvent("Notify",source,"importante","Número escolhido já possui um proprietário.",5000)
						end
					else
						TriggerClientEvent("Notify",source,"importante","O número telefônico deve conter 6 dígitos e somente números.",5000)
					end
				return end
	...
	if entity[1] ~= nil and entity[2] ~= nil and entity[3] ~= nil and entity[4] ~= nil then
		...
		if user_id and verifyObjects[user_id] == nil and activeItens[user_id] == nil then
			...
			if trashTables[model][netObjects] then
				TriggerClientEvent("Notify",source,"importante",nameEmpty.." vazia.",5000)
				return
			end
