...
RegisterNUICallback("activeFrequency",function(data,cb)
	if actionDelay >= GetGameTimer() then 
		TriggerEvent("Notify","negado","Aguarde alguns instantes...")
		return 
	end
	...
	if src.startFrequency(frequency) then 
		...
		TriggerEvent("Notify","default","Conectado <b>"..frequency..".00</b> MHz.",5000,"normal","atenção")
	else
		actionDelay = GetGameTimer() + 12000
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- INATIVEFREQUENCY
...
RegisterNUICallback("inativeFrequency",function(data,cb)
	...
	TriggerEvent("Notify","default","Desconectado",5000,"normal","atenção")
end)
-- -----------------------------------------------------------------------------------------------------------------------------------------
-- -- OUTSERVERS
-- -----------------------------------------------------------------------------------------------------------------------------------------
