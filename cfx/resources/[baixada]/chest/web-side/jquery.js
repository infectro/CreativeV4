			...
			if (data.myInventory[slot] !== undefined){
				...
				if (actualPercent <= 1)
					...
					<div class="item populated"
						title=""
						data-max="${v["max"]}" 
						data-type="${v["type"]}" data-serial="${v["serial"]}" data-amount="${v.amount}"
						data-peso="${v.peso}" data-item-key="${v.key}" data-name-key="${v.name}" data-slot="${slot}" data-description="${v["desc"]}" data-economy="${v["economy"]}">
						<div class="top">
							<span class="itemAmount">${formatarNumero(v.amount)}</span> <b>x</b> | 
							<span class="itemWeight">${(v.peso*v.amount).toFixed(2)}</span> <b>kg</b>
						</div>
						<img src="http://45.162.228.208/baixada/item/${v.index}.png">
						<div class="infoItem">
		...
		for (let x = 1; x <= 100; x++){
			...
			if (data.myChest[slot] !== undefined){
				...
				if (actualPercent <= 1)
					...
					<div class="item populated"
						title=""
						data-max="${v["max"]}" data-type="${v["type"]}" data-serial="${v["serial"]}" 
						data-amount="${v.amount}" data-peso="${v.peso}" data-item-key="${v.key}" 
						data-name-key="${v.name}" data-slot="${slot}" data-description="${v["desc"]}" data-economy="${v["economy"]}">
						<div class="top">
							<span class="itemAmount">${formatarNumero(v.amount)}</span> <b>x</b> | 
							<span class="itemWeight">${(v.peso*v.amount).toFixed(2)}</span> <b>kg</b>
						</div>
