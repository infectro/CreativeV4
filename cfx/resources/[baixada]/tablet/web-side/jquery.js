...
const benefactor = (mode) => {
	...
	$("#content").html(`
		<div id="benefactorBar">
			...
			<li id="benefactor" data-id="Aluguel" ${mode == "Aluguel" ? "class=active":""}>VEÍCULOS</li>
		...
		if (mode !== "Possuidos"){
			$("#pageVehicles").html(`
				${nameList.map((item) => (`<span>
					<left>
						${item["name"]}<br>
						<b>Valor:</b> ${mode == "Aluguel" ? format(item["price"])+" Cryptos":"$"+format(item["price"])}<br>
						<b>Taxa Semanal:</b> $${format(item["tax"])}<br>
						<b>Porta-Malas:</b> ${format(item["chest"])}Kg
					</left>
					<right>
						${mode == "Aluguel" ? "<div id=\"benefactorRental\" data-name="+item["k"]+">C</div><div id=\"benefactorRentalMoney\" data-name="+item["k"]+">V</div>":"<div id=\"benefactorBuy\" data-name="+item["k"]+">COMPRAR</div>"}
