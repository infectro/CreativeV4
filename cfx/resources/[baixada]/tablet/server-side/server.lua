	...
	if user_id then
		if actived[user_id] == nil then
			...
			if vRP.getFines(user_id) > 0 then
				TriggerClientEvent("Notify",source,"importante","Multas pendentes encontradas.",3000)
				actived[user_id] = nil
				return
			end
				...
				if vRP.paymentGems(user_id,vehPrice) then
					...
					if vehicle[1] then
						if parseInt(os.time()) >= (vehicle[1]["rental"] + 24 * vehicle[1]["rendays"] * 60 * 60) then
							...
							TriggerClientEvent("Notify",source,"sucesso","Adicionado <b>30 Dias</b> de aluguel no veículo <b>"..vehicleName(vehName).."</b>.",5000)
						end
					else
						...
						TriggerClientEvent("Notify",source,"sucesso","Aluguel do veículo <b>"..vehicleName(vehName).."</b> concluído.",5000)
					end
				else
...
function cRP.requestBuy(vehName)
	...
	if user_id then
		if actived[user_id] == nil then
			...
			local vehicle = vRP.query("vehicles/selectVehicles",{ user_id = parseInt(user_id), vehicle = vehName })
			if vehicle[1] then
				TriggerClientEvent("Notify",source,"importante","Já possui um <b>"..vehicleName(vehName).."</b>.",3000)
				actived[user_id] = nil
				return
			else
