...
AddEventHandler("target:toggleAdmin",function()
	if adminService then
		...
		setDistance = 99.0
		adminService = true
		TriggerEvent("Notify","sucesso","Sistema ativado.",5000)
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- THREADSYSTEM
...
Citizen.CreateThread(function()
	...
	AddCircleZone("polService01",vector3(1833.95,3677.97,34.19),0.75,{
		name = "polService01",
		heading = 297.64
	},{
		distance = 1.0,
	...
	AddCircleZone("polService04",vector3(-810.21,-1234.99,6.86),0.75,{
		name = "polService04",
		heading = 136.07
	},{
		distance = 1.0,
	...
	AddTargetModel({ 589548997,-945465914,1043035044,-655644382,865627822,862871082,729253480 },{
		options = {
			{
				event = "inventory:verifyObjects",
				label = "Consertar",
...
local twoPed = {
	{
		event = "player:startRelationship",
		label = "Pedir em Namoro",
		tunnel = "server"
	},
	{
		event = "player:endRelationship",
		label = "Terminar Namoro",
		tunnel = "server"
	}
}
-----------------------------------------------------------------------------------------------------------------------------------------
					...
					elseif IsEntityAVehicle(entity) then
						if #(coords - entCoords) <= 1.0 then
							...
							if policeService then
								...
								local distance03 = #(coords - vector3(1262.1,-2565.6,42.71))
								local distance04 = #(coords - vector3(602.37,-438.0,24.75))
								if distance01 > 20 and distance02 > 20 and distance03 > 20 and distance04 > 20 then
