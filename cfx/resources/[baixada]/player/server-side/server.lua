...
function cRP.receiveSalary()
	...
	if user_id then
		if vRP.userPremium(user_id) then
			...
			TriggerClientEvent("Notify",source,"importante","Salário de <b>$1.000</b> recebido.",5000)
		end

		if vRP.hasPermission(user_id,"Police") then
			vRP.addBank(user_id,2000)
			TriggerClientEvent("Notify",source,"importante","Salário de <b>$2.000</b> recebido.",5000)
		end
...
AddEventHandler("player:servicePolice",function()
	...
	if user_id then
		if vRP.hasPermission(user_id,"Police") then
			...
			TriggerClientEvent("Notify",source,"importante","Saiu de serviço.",5000)
		elseif vRP.hasPermission(user_id,"waitPolice") then
			...
			TriggerClientEvent("Notify",source,"importante","Entrou em serviço.",5000)
		end
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
		...
		if user_id and args[1] and parseInt(args[2]) > 0 then
			if vRP.hasPermission(user_id,"service"..args[1]) then
				...
				if args[1] == "Police" or args[1] == "Paramedic" then
					...
					TriggerClientEvent("Notify",source,"sucesso","Passaporte <b>"..parseFormat(args[2]).."</b> adicionado.",5000)
				else
		...
		if user_id and args[1] and parseInt(args[2]) > 0 then
			if vRP.hasPermission(user_id,"service"..args[1]) then
				if args[1] == "Police" or args[1] == "Paramedic" then
					...
					TriggerClientEvent("Notify",source,"importante","Passaporte <b>"..parseFormat(args[2]).."</b> removido.",5000)
				else
					vRP.remPermission(args[2],args[1])
					TriggerClientEvent("Notify",source,"importante","Passaporte <b>"..parseFormat(args[2]).."</b> removido.",5000)
				end
