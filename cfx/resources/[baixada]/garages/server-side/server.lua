		...
		if spawnTimers[user_id] == nil then
			...
			if vehNet == nil then
				...
				if os.time() >= parseInt(vehicle[1]["tax"] + 24 * 7 * 60 * 60) then
					TriggerClientEvent("Notify",source,"importante","Taxa do veículo atrasada, efetue o pagamento<br>através do seu tablet no sistema da benefactor.",5000)
				...
				elseif parseInt(vehicle[1]["arrest"]) >= 1 then
					...
					if parseInt(vehicle[1]["rental"]) > 0 then
						if parseInt(os.time()) >= (vehicle[1]["rental"] + 24 * vehicle[1]["rendays"] * 60 * 60) then
							TriggerClientEvent("Notify",source,"importante","Validade do veículo expirou, efetue a renovação do mesmo.",5000)
				...
				if GetGameTimer() >= searchTimers[user_id] then
					...
					if DoesEntityExist(idNetwork) and not IsPedAPlayer(idNetwork) then
						...
						TriggerClientEvent("Notify",source,"importante","Rastreador do veículo foi ativado por <b>30 segundos</b>, lembrando que<br>se o mesmo estiver em movimento a localização pode ser imprecisa.",10000)
					else
						...
						TriggerClientEvent("Notify",source,"sucesso","A seguradora efetuou o resgate do seu veículo e o mesmo já se encontra disponível para retirada.",5000)
					end
				else
					TriggerClientEvent("Notify",source,"importante","O rastreador só pode ser ativado a cada <b>60 segundos</b>.",5000)
				end
			end

			spawnTimers[user_id] = nil
		else
			TriggerClientEvent("Notify",source,"importante","Existe uma busca por seu veículo em andamento.",5000)
		end
	end
end
-----------------------------------------------------------------------------------------------------------------------------------------
	...
	if user_id then
		if vehPlates[vehPlate] == user_id then
			...
			if vehLock then
				...
				TriggerClientEvent("Notify",source,"sucesso","Veículo destrancado.",5000)
			else
				TriggerClientEvent("sounds:source",source,"lock",0.3)
				TriggerClientEvent("Notify",source,"sucesso","Veículo trancado.",5000)
			end
				...
				if consult[1] == nil then
					...
					if ownerConsult[1] then
						if parseInt(os.time()) >= parseInt(ownerConsult[1]["tax"] + 24 * 7 * 60 * 60) then
							TriggerClientEvent("Notify",source,"importante","Taxas da propriedade atrasada.",10000)
							return false
						end
					end
				end
			...
			if args[1] == "transfer" and parseInt(args[3]) > 0 then
				...
				if myVehicle[1] then
					if parseInt(myVehicle[1]["rental"]) >= 1 then
						TriggerClientEvent("Notify",source,"importante","Veículos alugados não podem ser transferidos.",5000)
						return
					end
					...
					if identity then
						...
						if vRP.request(source,"Deseja transferir o veículo <b>"..vehicleName(args[2]).."</b> para <b>"..identity["name"].." "..identity["name2"].."</b>?",30) then
							...
							if vehicle[1] then
								TriggerClientEvent("Notify",source,"importante","<b>"..identity["name"].." "..identity["name2"].."</b> já possui este modelo de veículo.",5000)
							else
								...
								TriggerClientEvent("Notify",source,"sucesso","Transferência concluída.",5000)
							end
						end
					end
				end
