	...
	if user_id then
		...
		if vehicle then
			if impoundVehs[vehName.."-"..vehPlate] == nil then
				...
				vRP.generateItem(user_id,itensList[math.random(#itensList)],parseInt(math.random(35,45)),true)
	...
	if user_id and vRP.getHealth(source) > 101 then
		if vRP.hasGroup(user_id,"Police") then
			...
			if impoundVehs[entity[2].."-"..entity[1]] == nil then
				...
				TriggerClientEvent("Notify",source,"sucesso","Veículo <b>"..vehicleName(entity[2]).."</b> foi registrado.",3000)
			else
				TriggerClientEvent("Notify",source,"importante","Veículo <b>"..vehicleName(entity[2]).."</b> já está na lista.",3000)
			end
		end
	end
end)
				...
				if plateUser then
					...
					if inVehicle[1] then
						if inVehicle[1]["arrest"] <= 0 then
							...
							TriggerClientEvent("Notify",source,"sucesso","Veículo apreendido.",5000)
						else
							TriggerClientEvent("Notify",source,"importante","Veículo já se encontra apreendido.",5000)
						end
					end
				end
			end
