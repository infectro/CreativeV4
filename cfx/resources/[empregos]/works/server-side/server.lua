	...
	if user_id then
		...
		if (vRP.inventoryWeight(user_id) + (itemWeight(deliveryItem) * parseInt(paymentAmount[user_id]))) <= vRP.getBackpack(user_id) then
			if vRP.tryGetInventoryItem(user_id,works[serviceName]["deliveryItem"],deliveryAmount[user_id]) then
				...
				TriggerClientEvent("Notify",source,"importante","Precisa de <b>"..parseFormat(deliveryAmount[user_id]).."x "..itemName(works[serviceName]["deliveryItem"]).."</b> para entregar.",5000)
			end
		else
			TriggerClientEvent("Notify",source,"negado","Mochila cheia.",5000)
		end
		...
		if vRP.hasGroup(user_id,works[serviceName]["perm"]) then
			if activeWorks[serviceName] then
				if parseInt(#activeWorks[serviceName]) < 5 then
					...
					TriggerClientEvent("Notify",source,"importante","Limite de trabalhadores atingido.",3000)
				end
			else
				return true
			end
...
AddEventHandler("works:bullguerBox",function()
	...
	if user_id then
		...
		if consultFood[1] <= 0 then
			TriggerClientEvent("Notify",source,"importante","Precisa de <b>1x "..itemName("bullguerfood").."</b>.",5000)
			return
		end

		local consultJuice = vRP.getInventoryItemAmount(user_id,"bullguerjuice")
		if consultJuice[1] <= 0 then
			TriggerClientEvent("Notify",source,"importante","Precisa de <b>1x "..itemName("bullguerjuice").."</b>.",5000)
			return
		end
