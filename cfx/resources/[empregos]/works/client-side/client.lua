					...
					if IsControlJustPressed(1,38) and inSeconds <= 0 then
						...
						if not inService then
							if vSERVER.checkPermission(k) then
								...
								TriggerEvent("Notify","importante","Serviço de <b>"..k.."</b> foi iniciado.",5000)
							end
						else
							if inService == k then
								lastService = k
								...
								TriggerEvent("Notify","importante","Serviço de <b>"..k.."</b> foi finalizado.",5000)
								...
								TriggerEvent("Notify","importante","Dirija-se até o emprego de <b>"..inService.."</b> e finalize o mesmo.",5000)
							end
						end
					end
				end
							...
							if works[inService]["routeCollect"] then
								if works[inService]["collectVehicle"] ~= nil then
									if GetEntityModel(GetPlayersLastVehicle()) == works[inService]["collectVehicle"] then
										...
										TriggerEvent("Notify","importante","Precisa utilizar o veículo do <b>"..inService.."</b>.",3000)
									end
								else
									if vSERVER.collectConsume(inService) then
										if inCollect >= #works[inService]["collectCoords"] then
