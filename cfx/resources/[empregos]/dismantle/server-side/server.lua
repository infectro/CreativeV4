...
function cRP.removeList(vehPlate,vehName)
	...
	if user_id then
		...
		if not plateUser then
			...
			TriggerClientEvent("Notify",source,"importante","Veículo é protegido pela seguradora.",1000)
		end
	end
...
AddEventHandler("dismantle:invokeList",function()
	...
	if user_id and timeList > 0 then
		...
		if vehListNames ~= "" then
			TriggerClientEvent("Notify",source,"importante",vehListNames.." a lista é atualizada em <b>"..parseInt(timeList).." minutos</b>, cada veículo entregue é removido da lista.",60000)
		else
