...
AddEventHandler("vrp:playerActive",function(user_id,surname)
	...
	SetDiscordAppId(820318459245756416)
    ...
    SetRichPresence(user_id.." - "..(surname or "N/A"))
			...
			if GetEntityHealth(ped) <= 101 then
				if not deathStatus then
					...
					if timeDeath <= 0 then
						...
						SendNUIMessage({ deathtext = "inconsciente <br> <small>aguarde <color>"..timeDeath.." segundos</color></small>" })
